...
var sourceLink = document.querySelector('.source-link');
var isolatedLink = document.querySelector('.isolated-link');

var canvas = document.querySelector('#canvas');
var chartsExport = document.querySelector('.charts-export');
...
/* Use local mess */
if(localStorage.getItem('uselocalmess') == 'checked') {
	uselocalmess.checked = true;
	go.innerHTML = '<i class="fi-refresh"></i> Process Isolated Data';
	isolatedLink.classList.add('visible');
}
if(localStorage.getItem('uselocalmess') == 'unchecked') {
	uselocalmess.checked = false;
	go.innerHTML = '<i class="fi-refresh"></i> Process Source Data';
}

uselocalmess.addEventListener('change', function(){

	if(this.checked) {
		localStorage.setItem('uselocalmess', 'checked');
		go.innerHTML = '<i class="fi-refresh"></i> Process Isolated Data';
	}
	else {
		localStorage.setItem('uselocalmess', 'unchecked');
		go.innerHTML = '<i class="fi-refresh"></i> Process Source Data';
	}
...
downloadfile.addEventListener('change', function(){
	if(this.value != '') {
		...
		// download.innerHTML = '<i class="fi-download"></i> Download '+name+'.csv';
	}
});
...
bgWorker.onmessage = function(e) {
	...
	if(e.data[0] == 'processlocal' || e.data[0] == 'processblocks') {
		...
		document.querySelector('.download-group').classList.add('visible'); 
		isolatedLink.classList.add('visible');
		canvas.classList.add('visible');
		chartsExport.classList.add('visible');
		setTimeout(function(){
...
function startProcess() {
	...
	document.querySelector('.download-group').classList.remove('visible');
...
function buildCharts(chartsinfo) {
	...
	var chartHeight = labels.length * 18;
	// if(chartHeight < 40) 
	chartHeight += 30;

	canvas.innerHTML = '<canvas height="'+chartHeight+'" id="overview"></canvas>';
	...
	var myChart = new Chart(ctx, {
	    ...
	    data: {
	        ...
	        datasets: [{
	            ...
	            backgroundColor: 'rgba(0, 209, 141, 1)', 
	            borderColor: 'rgba(0, 209, 141, 1)',
	            borderWidth: 1
	        }]
	    },
	    options: {
...
document.addEventListener('dragover', function(e){
	e.preventDefault();
	e.stopPropagation();
	html.classList.add('drag-mode');
}, false);

document.addEventListener('mouseleave', function(e){
	e.preventDefault();
	e.stopPropagation();
	html.classList.remove('drag-mode');
}, false);
...
if(dropZones.length > 0) dropZones.forEach(function(zone){
	...
	zone.addEventListener('drop', function(e){
		...
		var target = this.getAttribute('data-drop-target');
		reader.onload = function(event) {
			if(target) document.querySelector(target).value = event.target.result;            
		}        
